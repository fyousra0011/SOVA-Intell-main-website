...
function Navbar({ page, go }: { page: Page; go: (p: Page) => void }) {
  ...
  const goToPartners = () => {
    go("home");
    window.setTimeout(() => document.getElementById("partners")?.scrollIntoView({ behavior: "smooth" }), 180);
  };
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#0a0a0a]/92 backdrop-blur-md border-b border-[#1a1a1a]" : ""}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-16 md:h-[72px]">
          ...
          <div className="hidden md:flex items-center gap-7">
            ...
            <button onClick={goToPartners} className="text-[11px] uppercase tracking-[0.16em] text-gray-500 transition-colors hover:text-white">Partners</button>
      ...
      <div className={`fixed inset-0 z-40 bg-[#0a0a0a] flex flex-col pt-20 px-8 md:hidden transition-all duration-300 ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className="flex flex-col gap-6 pt-6">
          ...
          <button onClick={() => { goToPartners(); setOpen(false); }} className="text-left text-4xl font-black text-white uppercase tracking-tight hover:text-gray-300 transition-colors" style={{ fontFamily: DISPLAY }}>Partners</button>
          <button onClick={() => { go("rsvp"); setOpen(false); }} className="text-left text-4xl font-black text-white uppercase tracking-tight hover:text-gray-300 transition-colors" style={{ fontFamily: DISPLAY }}>RSVP</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
                {([["Home","home"],["About","about"],["Services","services"],["Sponsors","sponsors"],["RSVP","rsvp"]] as [string,Page][]).map(([l,p]) => (
          ...
          <div>
            ...
            <div className="flex gap-4 mb-5">
                {LINKEDIN_URL && <a href={LINKEDIN_URL} target="_blank" rel="noopener noreferrer" aria-label="SOVA Intelligence on LinkedIn" className="text-gray-600 hover:text-white transition-colors p-1"><Linkedin size={18} /></a>}
                {INSTAGRAM_URL && <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" aria-label="SOVA Intelligence on Instagram" className="text-gray-600 hover:text-white transition-colors p-1"><Instagram size={18} /></a>}
            </div>
...
function HomePage({ go }: { go: (p: Page) => void }) {
  ...
  return (
    <div style={{ fontFamily: BODY }}>
      ...
      {/* ── PARTNERS ──────────────────────────────────────────── */}
      <section id="partners" className="bg-[#111111] py-24 md:py-32">
...
function RSVPPage() {
  const [form, setForm]           = useState({ name:"", org:"", title:"", email:"", phone:"", queryType:"", message:"", website:"" });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!form.queryType) {
      setError("Please select the nature of your enquiry.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(response.status === 429
          ...
          : result.message || "We could not submit your enquiry. Please check your details and try again.");
      }
      setSubmitted(true);
    } catch (submissionError) {
      setError(submissionError instanceof Error
        ? submissionError.message
        : "We could not reach the SOVA team. Please try again later.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="border-t border-[#1a1a1a]">
          ...
          <FadeUp delay={420}>
            <div className="pt-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
              ...
              <button type="submit" disabled={submitting} className="flex-shrink-0 flex items-center gap-3 px-8 py-4 rounded-full font-bold text-sm uppercase tracking-[0.15em] text-white group hover:opacity-90 transition-opacity disabled:cursor-not-allowed disabled:opacity-60" style={{ background: GRAD }}>
                {submitting ? "Submitting..." : "Submit"} {!submitting && <Send size={13} className="group-hover:translate-x-1 transition-transform" />}
              </button>
            </div>
          </FadeUp>
